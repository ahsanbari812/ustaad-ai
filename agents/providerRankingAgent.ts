...
// Normalize location strings to match KARACHI_ZONES keys (e.g. "Federal B Area" → "federal_b_area")
function normalizeZoneKey(location: string): string {
...
function calculateDistance(loc1?: string, loc2?: string, staticFallback: number = 5): number {
  ...
  const coord1 = KARACHI_ZONES[normalizeZoneKey(loc1)];
  const coord2 = KARACHI_ZONES[normalizeZoneKey(loc2)];
